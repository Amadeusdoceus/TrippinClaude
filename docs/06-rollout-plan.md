...
| 3 | **Dados do cadastro (nome, telefone, CPF, nascimento) eram descartados quando a confirmação de e-mail está ligada** — `auth.signUp()` (`app/src/trippin-api.js`) só gravava esses campos em `public.profiles` quando o `signUp()` já devolvia sessão na hora; com "Confirm email" ligado no projeto de produção (é o caso hoje), a sessão só existe depois que a pessoa clica no link do e-mail, então o `profiles.update(...)` nunca rodava. | Toda conta criada em produção ficava com o perfil vazio (nome, telefone etc. em branco) depois de confirmar o e-mail — reportado como "conta criada mas não consigo acessar" em teste manual. | ✅ **Resolvido em 2026-09-19.** Os campos do cadastro agora ficam guardados em `localStorage` (por e-mail) enquanto a confirmação está pendente e são aplicados por `currentUser()` assim que a sessão existir de verdade (login manual ou sessão restaurada no boot). Coberto por `tests/e2e/auth-remote.spec.js` ("dados do cadastro não se perdem quando a confirmação de e-mail é obrigatória"). |
| 4 | **App não reconhecia uma sessão do Supabase já válida ao carregar** — `App()` (`app/index.html`) decidia a tela inicial só pelo usuário salvo no cache local do próprio app (`persisted.user`), nunca checando se já existia uma sessão remota válida no SDK do Supabase. Quem clicava no link de confirmação de e-mail (ou reabria o app numa aba/instalação nova com sessão ainda válida) caía de volta na tela "Escolha o idioma" como se não tivesse feito nada. | Combinado com o item 3, dava a impressão de que a conta/o cadastro simplesmente não funcionava — bloqueou testes manuais em 2026-09-19. | ✅ **Resolvido em 2026-09-19.** `App()` agora chama `TrippinAPI.auth.currentUser()` assim que `apiMode` resolve para `'remote'` e não há usuário local; se houver sessão válida, entra direto no Home em vez de ficar preso no onboarding. Coberto por `tests/e2e/auth-remote.spec.js` ("com sessão do Supabase já válida, o app pula o onboarding e vai direto pro Home"). |
