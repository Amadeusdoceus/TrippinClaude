// ============================================================================
// Trippin — Edge Function: send-invite (versão endurecida)
// ----------------------------------------------------------------------------
// Substitui backend/supabase/functions/send-invite/index.ts.
// Corrige o finding C-02 da auditoria: a versão original não verificava
// autenticação nenhuma. Como a chave publishable está no repositório por
// design, qualquer pessoa na internet podia disparar e-mails com a identidade
// visual do Trippin e gravar linhas em `invites` usando service_role.
//
// O que mudou:
...
//   4. sender_name vem do perfil autenticado, não do corpo.
//   5. Rate limit por usuário/hora, via public.rate_limits.
//   6. CORS por allowlist, não `*` (finding M-01).
//   7. Sem e-mail pessoal embutido — falha explícita se o secret faltar (M-05).
//   8. Token com 256 bits de entropia e expiração explícita.
//
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: isAdmin, error: adminErr } = await userClient.rpc("is_trip_admin_public", {
      p_trip_id: trip_id,
    });
    ...
    const { data: senderProfile } = await admin.from("profiles")
      .select("name").eq("id", callerId).maybeSingle();
    const senderName = senderProfile?.name?.trim() || "Um integrante";
    ...
    if (existingProfile) {
      const { data: alreadyMember } = await admin.from("trip_members")
        .select("user_id").eq("trip_id", trip_id).eq("user_id", existingProfile.id).maybeSingle();
    ...
    // ---- 7. Token com entropia real e expiração -------------------------
...
// ----------------------------------------------------------------------------
// Wrapper público de private.is_trip_admin, necessário porque o schema `private`
// não é exposto pela API REST. Adicionar numa migration:
//
//   create or replace function public.is_trip_admin_public(p_trip_id uuid)
//   returns boolean language sql security definer set search_path = '' stable as $$
//     select private.is_trip_admin(p_trip_id);
//   $$;
//   revoke execute on function public.is_trip_admin_public(uuid) from public, anon;
//   grant  execute on function public.is_trip_admin_public(uuid) to authenticated;
// ----------------------------------------------------------------------------
